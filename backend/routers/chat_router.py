...
async def get_current_user_dev_optional(request: Request, session: Session = Depends(get_session)):
    ...
    if DEV_MODE:
        ...
        areesha_user = session.exec(
            select(User).where(User.email == "areeshasattar5544@gmail.com")
        ).first()
...
async def send_message(
    ...
    ✅ ENHANCED: Language support added
    """
    print("\n" + "=" * 60)
    ...
    language = 'en'  # default
    try:
        if message_create.metadata_json:
            import json
            metadata = json.loads(message_create.metadata_json)
            language = metadata.get('language', 'en')
            print(f"🌍 Language preference: {language}")
    except Exception as e:
        print(f"⚠️ Failed to extract language: {e}")
        pass
    ...
    try:
        ...
        # ✅ Process message with AI - PASS LANGUAGE
...
async def create_conversation(
    request: Request,
    conversation_create: ConversationCreate = None,  # Allow None initially
    session: Session = Depends(get_session),
    current_user: dict = Depends(get_current_user_dev_optional)
):
    """
    ...
    try:
        ...
        # Handle case where no body is provided or body is empty
        if conversation_create is None:
            from schemas import ConversationCreate as ConvCreateSchema
            conversation_create = ConvCreateSchema(title="New Chat", is_archived=False)

        print(f"📝 Body received: {conversation_create}")
...
async def detect_language(
    ...
    try:
        ...
        # Simple heuristic to detect Urdu text (Arabic script characters)
        # Urdu contains Arabic script characters in Unicode range \u0600-\u06FF
        urdu_chars = 0
        total_chars = 0
