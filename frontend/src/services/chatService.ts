...
class ChatService {
    ...
    try {
      ...
      const response = await fetch(`${this.baseUrl}/api/chat/message`, {
        ...
        body: JSON.stringify({
          ...
          content: content,
          role: "user",
          metadata_json: JSON.stringify({ language, voiceInput }),
        }),
      });
  ...
  // ✅ FIXED: Send proper body with title and is_archived
  async createConversation(title: string = "New Chat"): Promise<Conversation> {
    try {
      console.log("🔵 Creating conversation with title:", title);

      const response = await fetch(
        `${this.baseUrl}/api/chat/conversations/new`,
        {
          method: "POST",
          headers: this.getHeaders(),
          body: JSON.stringify({
  ...
  async getConversationMessages(
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(
          errorData.detail || "Failed to get conversation messages"
        );
      }
  ...
  async detectLanguage(
    text: string
  ): Promise<{ language: "en" | "ur"; supported: boolean }> {
    try {
      const response = await fetch(
        `${this.baseUrl}/api/chat/detect-language?text=${encodeURIComponent(
          text
        )}`,
        {
          method: "POST",
